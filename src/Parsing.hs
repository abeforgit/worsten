...
convert ts =  ts >>= doconvert
    where doconvert (x, s) = [(x, y, c) | (y, c) <- zip [0..] s]

parseToEnts:: String -> [Entity]
parseToEnts s =  convert (zippify s) >>= doParse
    where doParse (x, y, c) = strToEnt (y `div` 2) x  c
...
strToEnt x y '∩' =
    [SausageEnt {location = IVec x y,
    sausagePiece = SausagePiece north Up (Raw, Raw)}]
strToEnt x y '∪' =
    [SausageEnt {location = IVec x y,
    sausagePiece = SausagePiece south Up (Raw, Raw)}]
strToEnt x y '⊃' =
    [SausageEnt {location = IVec x y,
    sausagePiece = SausagePiece east Up (Raw, Raw)}]
strToEnt x y '⊂' =
    [SausageEnt {location = IVec x y,
    sausagePiece = SausagePiece west Up (Raw, Raw)}]
strToEnt x y _ = []
